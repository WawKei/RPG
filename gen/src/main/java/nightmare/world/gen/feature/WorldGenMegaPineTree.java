...
{
    private static final Block field_181633_e = Block.get(Block.LOG, BlockWood.SPRUCE);
    private static final Block field_181634_f = Block.get(Block.LEAVES, BlockLeaves.SPRUCE);
    private static final Block field_181635_g = Block.get(Block.PODZOL);
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        else
        {
            ...
            for (int j = 0; j < i; ++j)
            {
                Block block = worldIn.getBlockState(position.up(j));
                ...
                if (j < i - 1)
                {
                    block = worldIn.getBlockState(position.add(1, j, 0));
                    ...
                    block = worldIn.getBlockState(position.add(1, j, 1));
                    ...
                    block = worldIn.getBlockState(position.add(0, j, 1));
    ...
    private void func_175934_c(World worldIn, BlockPos p_175934_2_)
    {
        for (int i = 2; i >= -3; --i)
        {
            ...
            if (block.getId() == Block.GRASS || block.getId() == Block.DIRT)
            {
                this.setBlockAndNotifyAdequately(worldIn, blockpos, field_181635_g);
                break;
            }

            if (block.getId() != Block.AIR && i < 0)
            {
                break;
            }
        }
