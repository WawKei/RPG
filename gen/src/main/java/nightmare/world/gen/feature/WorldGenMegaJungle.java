...
{
    public WorldGenMegaJungle(boolean p_i46448_1_, int p_i46448_2_, int p_i46448_3_, Block p_i46448_4_, Block p_i46448_5_)
    {
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        else
        {
            ...
            for (int i2 = 0; i2 < i; ++i2)
            {
                ...
                if (this.func_150523_a(worldIn.getBlockState(blockpos)))
                {
                    this.setBlockAndNotifyAdequately(worldIn, blockpos, this.woodMetadata);

                    if (i2 > 0)
                    {
                        this.func_181632_a(worldIn, rand, blockpos.west(), EnumFacing.EAST);
                        this.func_181632_a(worldIn, rand, blockpos.north(), EnumFacing.SOUTH);
                    }
                }
                ...
                if (i2 < i - 1)
                {
                    ...
                    if (this.func_150523_a(worldIn.getBlockState(blockpos1)))
                    {
                        this.setBlockAndNotifyAdequately(worldIn, blockpos1, this.woodMetadata);

                        if (i2 > 0)
                        {
                            this.func_181632_a(worldIn, rand, blockpos1.east(), EnumFacing.WEST);
                            this.func_181632_a(worldIn, rand, blockpos1.north(), EnumFacing.SOUTH);
                        }
                    }

                    BlockPos blockpos2 = blockpos.south().east();

                    if (this.func_150523_a(worldIn.getBlockState(blockpos2)))
                    {
                        this.setBlockAndNotifyAdequately(worldIn, blockpos2, this.woodMetadata);

                        if (i2 > 0)
                        {
                            this.func_181632_a(worldIn, rand, blockpos2.east(), EnumFacing.WEST);
                            this.func_181632_a(worldIn, rand, blockpos2.south(), EnumFacing.NORTH);
                        }
                    }

                    BlockPos blockpos3 = blockpos.south();

                    if (this.func_150523_a(worldIn.getBlockState(blockpos3)))
                    {
                        this.setBlockAndNotifyAdequately(worldIn, blockpos3, this.woodMetadata);

                        if (i2 > 0)
                        {
                            this.func_181632_a(worldIn, rand, blockpos3.west(), EnumFacing.EAST);
                            this.func_181632_a(worldIn, rand, blockpos3.south(), EnumFacing.NORTH);
                        }
                    }
                }
            }
    ...
    private void func_181632_a(World p_181632_1_, Random p_181632_2_, BlockPos p_181632_3_, EnumFacing p_181632_4_)
    {
        if (p_181632_2_.nextInt(3) > 0 && p_181632_1_.isAirBlock(p_181632_3_))
        {
            this.setBlockAndNotifyAdequately(p_181632_1_, p_181632_3_, Block.get(Block.VINE, p_181632_4_.getIndex()));
        }
    }
