...
{
    private static final Block field_181653_a = Block.get(Block.LOG, BlockWood.OAK);
    private static final Block field_181654_b = Block.get(Block.LEAVES, BlockLeaves.OAK);
    ...
    public WorldGenTrees(boolean p_i46446_1_, int p_i46446_2_, Block p_i46446_3_, Block p_i46446_4_, boolean p_i46446_5_)
    {
        super(p_i46446_1_);
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= 256)
        {
            for (int j = position.getY(); j <= position.getY() + 1 + i; ++j)
            {
                ...
                for (int l = position.getX() - k; l <= position.getX() + k && flag; ++l)
                {
                    for (int i1 = position.getZ() - k; i1 <= position.getZ() + k && flag; ++i1)
                    {
                        if (j >= 0 && j < 256)
                        {
                            if (!this.func_150523_a(worldIn.getBlockState(blockpos$mutableblockpos.set(l, j, i1))))
                            {
                                flag = false;
                            }
                        }
            ...
            else
            {
                ...
                if ((block1.getId() == Block.GRASS || block1.getId() == Block.DIRT || block1.getId() == Block.FARMLAND) && position.getY() < 256 - i - 1)
                {
                        ...
                        for (int k1 = position.getX() - j1; k1 <= position.getX() + j1; ++k1)
                        {
                            ...
                            for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2)
                            {
                                ...
                                if (Math.abs(l1) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0)
                                {
                                    ...
                                    if (block.getId() == Block.AIR || BlockHelper.isLeave(block) || block.getId() == Block.VINE)
                                    {
                    ...
                    {
                        Block block2 = worldIn.getBlockState(position.up(j3));

                        if (block2.getId() == Block.AIR || BlockHelper.isLeave(block2) || block2.getId() == Block.VINE)
                        {
                            ...
                            if (this.vinesGrow && j3 > 0)
                            {
                                ...
                                {
                                    this.func_181651_a(worldIn, position.add(-1, j3, 0), EnumFacing.EAST.getIndex());
                                }

                                if (rand.nextInt(3) > 0 && worldIn.isAirBlock(position.add(1, j3, 0)))
                                {
                                    this.func_181651_a(worldIn, position.add(1, j3, 0), EnumFacing.WEST.getIndex());
                                }

                                if (rand.nextInt(3) > 0 && worldIn.isAirBlock(position.add(0, j3, -1)))
                                {
                                    this.func_181651_a(worldIn, position.add(0, j3, -1), EnumFacing.SOUTH.getIndex());
                                }

                                if (rand.nextInt(3) > 0 && worldIn.isAirBlock(position.add(0, j3, 1)))
                                {
                                    this.func_181651_a(worldIn, position.add(0, j3, 1), EnumFacing.NORTH.getIndex());
                                }
                            }
                        }
                    }
                            ...
                            for (int l4 = position.getX() - k4; l4 <= position.getX() + k4; ++l4)
                            {
                                for (int i5 = position.getZ() - k4; i5 <= position.getZ() + k4; ++i5)
                                {
                                    ...
                                    if (BlockHelper.isLeave(worldIn.getBlockState(blockpos$mutableblockpos1)))
                                    {
                                        ...
                                        {
                                            this.func_181650_b(worldIn, blockpos2, EnumFacing.EAST.getIndex());
                                        }

                                        if (rand.nextInt(4) == 0 && worldIn.getBlockState(blockpos3).getId() == Block.AIR)
                                        {
                                            this.func_181650_b(worldIn, blockpos3, EnumFacing.WEST.getIndex());
                                        }

                                        if (rand.nextInt(4) == 0 && worldIn.getBlockState(blockpos4).getId() == Block.AIR)
                                        {
                                            this.func_181650_b(worldIn, blockpos4, EnumFacing.SOUTH.getIndex());
                                        }

                                        if (rand.nextInt(4) == 0 && worldIn.getBlockState(blockpos1).getId() == Block.AIR)
                                        {
                                            this.func_181650_b(worldIn, blockpos1, EnumFacing.NORTH.getIndex());
                                        }
                                    }
                                }
                            }
    ...
    {
        this.setBlockAndNotifyAdequately(p_181652_1_, p_181652_3_, Block.get(Block.COCOA, p_181652_4_.getIndex()));//TODO AGE p_181652_2_
    }

    private void func_181651_a(World p_181651_1_, BlockPos p_181651_2_, int p_181651_3_)
    {
        this.setBlockAndNotifyAdequately(p_181651_1_, p_181651_2_, Block.get(Block.VINE, p_181651_3_));
    }

    private void func_181650_b(World p_181650_1_, BlockPos p_181650_2_, int p_181650_3_)
    {
        this.func_181651_a(p_181650_1_, p_181650_2_, p_181650_3_);
        int i = 4;

        for (p_181650_2_ = p_181650_2_.down(); p_181650_1_.getBlockState(p_181650_2_).getId() == Block.AIR && i > 0; --i)
        {
            this.func_181651_a(p_181650_1_, p_181650_2_, p_181650_3_);
