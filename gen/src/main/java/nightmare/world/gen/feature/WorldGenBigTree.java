...
public class WorldGenBigTree extends WorldGenAbstractTree
{
    ...
    void func_181631_a(BlockPos p_181631_1_, float p_181631_2_, Block p_181631_3_)
    {
        ...
        for (int j = -i; j <= i; ++j)
        {
            for (int k = -i; k <= i; ++k)
            {
                if (Math.pow((double)Math.abs(j) + 0.5D, 2.0D) + Math.pow((double)Math.abs(k) + 0.5D, 2.0D) <= (double)(p_181631_2_ * p_181631_2_))
                {
                    ...
                    Block material = this.world.getBlockState(blockpos);

                    if (material.getId() == Block.AIR || BlockHelper.isLeave(material))
                    {
    ...
    void generateLeafNode(BlockPos pos)
    {
        ...
        {
            this.func_181631_a(pos.up(i), this.leafSize(i), Block.get(Block.LEAVES));
        }
    }
    ...
    void func_175937_a(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_)
    {
        ...
        for (int j = 0; j <= i; ++j)
        {
            ...
            int blocklog$enumaxis = this.func_175938_b(p_175937_1_, blockpos1);
            short[] faces = new short[]{
                    0,
                    0b1000,
                    0b0100
            };
            this.setBlockAndNotifyAdequately(this.world, blockpos1, Block.get(p_175937_3_.getId(), ((p_175937_3_.getDamage() & 0x03) | faces[blocklog$enumaxis])));
        }
    }
    ...
    private int func_175938_b(BlockPos p_175938_1_, BlockPos p_175938_2_)
    {
        int blocklog$enumaxis = 0;
    ...
    void generateTrunk()
    {
        ...
        Block block = Block.get(Block.LOG);
    ...
    void generateLeafNodeBases()
    {
        for (FoliageCoordinates worldgenbigtree$foliagecoordinates : this.field_175948_j)
        {
            ...
            {
                this.func_175937_a(blockpos, worldgenbigtree$foliagecoordinates, Block.get(Block.LOG));
            }
        }
    }
    ...
    int checkBlockLine(BlockPos posOne, BlockPos posTwo)
    {
        ...
        else
        {
            for (int j = 0; j <= i; ++j)
            {
                ...
                if (!this.func_150523_a(this.world.getBlockState(blockpos1)))
                {
                    return j;
                }
            }
    ...
    {
        Block block = this.world.getBlockState(this.basePos.down());

        if (block.getId() != Block.DIRT && block.getId() != Block.GRASS && block.getId() != Block.FARMLAND)
        {
            return false;
        }
        else
