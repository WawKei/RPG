package rpg.entityManager.entity;
...
public abstract class Entity{
	...
	public Entity ridingEntity = null;
	...
	public Entity(Level worldIn, Channel channel){
		this.channel = channel;

		this.entityId = cn.nukkit.entity.Entity.entityCount++;
		...
		this.propertyManager.setLong(DATA_FLAGS, 0);
		this.propertyManager.setShort(DATA_MAX_AIR, 400);
		this.propertyManager.setString(DATA_NAMETAG, "");
		this.propertyManager.setLong(DATA_LEAD_HOLDER_EID, -1);
		this.propertyManager.setFloat(DATA_SCALE, 1);
	...
	protected void setSize(float width, float height) {
		if(width != this.width || height != this.height){
			...
			this.propertyManager.setFloat(DATA_BOUNDING_BOX_WIDTH, width);
			this.propertyManager.setFloat(DATA_BOUNDING_BOX_HEIGHT, height);
		}
	}
	...
	protected void updateFallState(float y, bool onGroundIn, Block blockIn, Vector3 pos){
		if(onGroundIn){
	...
	public boolean moveFlying(float strafe, float forward, float friction){
	...
	public void moveToBlockPosAndAngles(Vector3 pos, float rotationYawIn, float rotationPitchIn){
		this.setLocationAndAngles(pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, rotationYawIn, rotationPitchIn);
	}

	public void setLocationAndAngles(float x, float y, float z, float yaw, float pitch){
	...
	public float getDistanceSqToVector3(Vector3 vec){
		d0 = this.posX - vec.x;
		d1 = this.posY - vec.y;
		d2 = this.posZ - vec.z;
		return d0 * d0 + d1 * d1 + d2 * d2;
	...
	public float getDistanceToVector3(Vector3 vec){
		d0 = this.posX - vec.x;
		d1 = this.posY - vec.y;
		d2 = this.posZ - vec.z;
		return sqrt(d0 * d0 + d1 * d1 + d2 * d2);
	...
	public float getDistanceSqToEntity(Entity entityIn){
	...
	public float applyEntityCollision(Entity entityIn){
	...
	public void mountEntity(Entity entityIn){
		...
		if(entityIn == null){
			...
			this.setGenericFlag(DATA_FLAG_RIDING, false);
		}else{
			if(this.ridingEntity !== null){
				this.ridingEntity.riddenByEntity = null;
			}
			...
			this.setGenericFlag(DATA_FLAG_RIDING);
		}
	}
	...
	protected ArrayList<Player> hasSpawned = new ArrayList<Player>();
	...
	public void sendDataAll(array data){
		pk = new SetEntityDataPacket();
		pk.entityRuntimeId = this.entityId;
		pk.metadata = data != null ? data : this.dataProperties;
	...
	public vpod sendData(ArrayList<Player> player, array data){
	...
	public boolean setDataProperty(int id, int type, int value, boolean send){
	...
	public function setDataFlag(int propertyId, int id, boolean value, int type = DATA_TYPE_LONG){
	...
	public function getGenericFlag(int flagId) : bool{
		return this.getDataFlag(DATA_FLAGS, flagId);
	}
	...
	public function setGenericFlag(int flagId, bool value = true){
		this.setDataFlag(DATA_FLAGS, flagId, value, DATA_TYPE_LONG);
	}
