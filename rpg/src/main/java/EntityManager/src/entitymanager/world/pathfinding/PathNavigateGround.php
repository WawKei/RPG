...
class PathNavigateGround extends PathNavigate{
	...
	protected function getEntityPosition(){
		return new Vector3($this->theEntity->posX, $this->getPathablePosY(), $this->theEntity->posZ);
	}

	private function getPathablePosY(){
		if($this->theEntity->isInWater() && $this->getCanSwim()){
			$i = (int)$this->theEntity->getEntityBoundingBox()->minY;
			$block = $this->worldObj->getBlock(new Vector3(floor($this->theEntity->posX), $i, floor($this->theEntity->posZ)));
			$j = 0;
	...
	protected function removeSunnyPath(){
		...
		/*if ($this->shouldAvoidSun){
			if ($this->worldObj->canSeeSky(new BlockPos(floor($this->theEntity->posX), $this->theEntity->getEntityBoundingBox()->minY + 0.5, floor($this->theEntity->posZ)))){
				return;
			}
			...
			for($k = $i; $k < $i + $sizeX; ++$k){
				for($l = $j; $l < $j + $sizeZ; ++$l){
					...
					if($d0 * $p_179683_8_ + $d1 * $p_179683_10_ >= 0.0){
						...
						if($block->getId() == Block::WATER && !$this->theEntity->isInWater()){
							return false;
						}
