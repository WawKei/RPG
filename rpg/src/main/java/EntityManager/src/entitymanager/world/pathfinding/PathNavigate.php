...
abstract class PathNavigate{
		...
		if(!$this->noPath()){
			...
			if(!$this->noPath()){
				...
				if($vec32 != null){
					...
					$list = $this->theEntity->channel->getEntitiesWithinAABB($this->theEntity, $axisalignedbb1->addCoord(0.0, -1.0, 0.0));
					$d0 = -1.0;
					$axisalignedbb1 = $axisalignedbb1->offset(0.0, 1.0, 0.0);

					foreach($list as $entity){
						$d0 = $entity->getEntityBoundingBox()->calculateYOffset($axisalignedbb1, $d0);
					}
	...
	protected function pathFollow(){
		/*for($j = $this->currentPath->getCurrentPathIndex(); $j < $this->currentPath->getCurrentPathLength(); ++$j){
			...
			$this->theEntity->worldObj->addParticle(new LavaDripParticle(new Vector3($c->xCoord + 0.5, $c->yCoord + 2, $c->zCoord + 0.5)));
		}*/
		$vec3 = $this->getEntityPosition();
		...
		$j1 = ceil($this->theEntity->width);
		$k1 = (int)$this->theEntity->height + 1;
		$l = $j1;
