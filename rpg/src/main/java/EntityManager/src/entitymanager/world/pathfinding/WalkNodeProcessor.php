...
namespace EntityManager\world\pathfinding;
...
class WalkNodeProcessor extends NodeProcessor{
	...
	public function getPathPointTo($entityIn){
		if($this->canSwim && $entityIn->isInWater()){
			...
			$i = floor($entityIn->getEntityBoundingBox()->minY + 0.5);
		}

		return $this->openPoint(floor($entityIn->getEntityBoundingBox()->minX), $i, floor($entityIn->getEntityBoundingBox()->minZ));
	}
	...
	public static function func_176170_a($worldObj, $entityIn, $x, $y, $z, $sizeX, $sizeY, $sizeZ, $avoidWater, $breakDoors, $enterDoors){
		$flag = false;
		$blockpos = new Vector3($entityIn->posX, $entityIn->posY, $entityIn->posZ);
			...
			for($j = $y; $j < $y + $sizeY; ++$j){
				for($k = $z; $k < $z + $sizeZ; ++$k){
					...
					if($block->getId() != Block::AIR && !($block instanceof Flowable)){
						...
						/*if($entityIn->worldObj->getBlock($mutableblockpos) instanceof Rail){
							if(!($entityIn->worldObj->getBlock($blockpos) instanceof Rail) && !($entityIn->worldObj->getBlock($blockpos->getSide(Vector3::SIDE_DOWN)) instanceof Rail)){
								return -3;
							}
						}else */if(!$block->canPassThrough() && (!$breakDoors || !($block instanceof Door)/* || block.getMaterial() != Material.wood*/)){
