...
class RandomPositionGenerator{
	...
	public static function findRandomTargetBlockAwayFrom($entitycreatureIn, $xz, $y, $targetVec3){
		RandomPositionGenerator::$staticVector = (new Vector3($entitycreatureIn->posX, $entitycreatureIn->posY, $entitycreatureIn->posZ))->subtract($targetVec3);
	...
	private static function findRandomTargetBlock($entitycreatureIn, $xz, $y, $targetVec3){
		...
		if($entitycreatureIn->hasHome()){
			$d0 = $entitycreatureIn->getHomePosition()->distanceSquared(new Vector3(floor($entitycreatureIn->posX), floor($entitycreatureIn->posY), floor($entitycreatureIn->posZ))) + 4.0;
		...
		for($j1 = 0; $j1 < 10; ++$j1){
			...
			if($targetVec3 == null || $l * $targetVec3->x + $i1 * $targetVec3->z >= 0.0){
				if ($entitycreatureIn->hasHome() && $xz > 1){
					...
					if ($entitycreatureIn->posX > $blockpos->x){
						$l -= rand(0, $xz / 2 - 1);
					}else{
						$l += rand(0, $xz / 2 - 1);
					}

					if ($entitycreatureIn->posZ > $blockpos->z){
						$i1 -= rand(0, $xz / 2 - 1);
					}else{
						$i1 += rand(0, $xz / 2 - 1);
					}
				...
				$l = $l + floor($entitycreatureIn->posX);
				$k1 = $k1 + floor($entitycreatureIn->posY);
				$i1 = $i1 + floor($entitycreatureIn->posZ);
